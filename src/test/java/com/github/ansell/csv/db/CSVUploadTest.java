...
public class CSVUploadTest {
    ...
    public void setUp() throws Exception {
        ...
        System.out
                .println("Creating Derby database instance for test: " + testName.getMethodName());
            ...
            try (final Statement deleteTable = conn.createStatement();) {
                ...
                try {
                    ...
                    try {
                        ...
                        assertEquals("Database '" + testName.getMethodName() + "' shutdown.",
                                e.getMessage());
                    } finally {
    ...
    public final void testMainEmpty() throws Exception {
        final Path testFile = testDir.resolve("test-empty.csv");
        Files.copy(this.getClass().getResourceAsStream("/com/github/ansell/csvsum/test-empty.csv"),
                testFile);
        final Path testMapping = testDir.resolve("test-mapping-empty.csv");
        Files.copy(this.getClass().getResourceAsStream(
                "/com/github/ansell/csvupload/test-mapping-empty.csv"), testMapping);
        ...
        CSVUpload.main("--database", databaseConnectionString, "--table", tableString, "--input",
                testFile.toAbsolutePath().toString(), "--mapping",
    ...
    public final void testMainSingleHeaderNoLines() throws Exception {
        final Path testFile = testDir.resolve("test-single-header.csv");
        Files.copy(this.getClass()
                .getResourceAsStream("/com/github/ansell/csvsum/test-single-header.csv"), testFile);
        final Path testMapping = testDir.resolve("test-mapping-single-header.csv");
        Files.copy(
                this.getClass().getResourceAsStream(
                        "/com/github/ansell/csvupload/test-mapping-single-header.csv"),
                testMapping);
    ...
    public final void testMainSingleHeaderOneLine() throws Exception {
        final Path testFile = testDir.resolve("test-single-header-one-line.csv");
        Files.copy(this.getClass().getResourceAsStream(
                "/com/github/ansell/csvsum/test-single-header-one-line.csv"), testFile);
        final Path testMapping = testDir.resolve("test-mapping-single-header-one-line.csv");
        Files.copy(
                this.getClass().getResourceAsStream(
                        "/com/github/ansell/csvupload/test-mapping-single-header-one-line.csv"),
                testMapping);
    ...
    public final void testMainMultiKey() throws Exception {
        final Path testFile = testDir.resolve("test-source-multi-key.csv");
        Files.copy(this.getClass().getResourceAsStream(
                "/com/github/ansell/csvjoin/test-source-multi-key.csv"), testFile);
        final Path testMapping = testDir.resolve("test-mapping-source-multi-key.csv");
        Files.copy(
                this.getClass().getResourceAsStream(
                        "/com/github/ansell/csvupload/test-mapping-source-multi-key.csv"),
                testMapping);
    ...
    public final void testMainSingleHeaderTwentyOneLines() throws Exception {
        final Path testFile = testDir.resolve("test-single-header-twenty-one-lines.csv");
        Files.copy(
                this.getClass().getResourceAsStream(
                        "/com/github/ansell/csvsum/test-single-header-twenty-one-lines.csv"),
                testFile);
        final Path testMapping = testDir.resolve("test-mapping-single-header-twenty-one-lines.csv");
        Files.copy(this.getClass().getResourceAsStream(
                "/com/github/ansell/csvupload/test-mapping-single-header-twenty-one-lines.csv"),
                testMapping);
